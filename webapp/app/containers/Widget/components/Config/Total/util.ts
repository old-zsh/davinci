...
import { NumericUnit, TotalTypes } from './constants'
...
export function getFormattedValue (value: number | string, format: IFieldTotalConfig) {
  ...
  switch (formatType) {
    ...
    case FieldFormatTypes.Currency:
      const {
        decimalPlaces,
        unit,
        useThousandSeparator } = config as IFieldTotalConfig['numeric'] | IFieldTotalConfig['currency']
      ...
      if (formatType === FieldFormatTypes.Currency) {
        const { prefix, suffix } = config as IFieldTotalConfig['currency']
    ...
    case FieldFormatTypes.Percentage:
      ...
      formattedValue = isNaN(formattedValue) ? value
        : `${formatByDecimalPlaces(formattedValue, (config as IFieldTotalConfig['percentage']).decimalPlaces)}%`
      break
    case FieldFormatTypes.ScientificNotation:
      formattedValue = (+value).toExponential((config as IFieldTotalConfig['scientificNotation']).decimalPlaces)
      formattedValue = isNaN(formattedValue) ? value : formattedValue
      break
    case FieldFormatTypes.Date:
      const { format } = config as IFieldTotalConfig['date']
